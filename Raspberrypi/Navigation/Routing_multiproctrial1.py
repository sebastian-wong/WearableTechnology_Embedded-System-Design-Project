...
class Map:
	...
	prev_step_compass_reading = None
		...
		print 'in calcDisplacement11111\n\n\n'
		...
		if ( prev_step_compass_reading is not None ) and ( abs(prev_step_compass_reading - heading) > 60 ):
			distance = 0
		prev_step_compass_reading = heading
...
def receiveUserInput(userInputInitialise, speechInput):
	...
	if voiceInput:
		...
		userInput = ''
		for i in range(len(numpadData)):
			userInput = userInput + numpadData[i]
...
if __name__ == '__main__':
	...
	askLevelNumber = 'please enter level of building'
	speaking_proc = Process(target=speaker.threadedFeedback, args=(askLevelNumber,))
	speaking_proc.start()
	speaking_proc.join()

	#level = raw_input()
	level = receiveUserInput(userInputInitialise, speechInput)
	...
	askStartPlace = 'please enter starting location'
	speaking_proc = Process(target=speaker.threadedFeedback, args=(askStartPlace,))
	speaking_proc.start()
	speaking_proc.join()

        #startPlace = raw_input()
	startPlace = receiveUserInput(userInputInitialie, speechInput)
	startPlace = confirmUserInput('confirmStartQuery', startPlace, speechInput, speaker, userInputInitialise)
	
	askDestPlace = 'please enter destination location'
	speaking_proc = Process(target=speaker.threadedFeedback, args=(askDestPlace,))
	speaking_proc.start()
	speaking_proc.join()

	#destPlace = raw_input()
	destPlace = receiveUserInput(userInputInitialise, speechInput)
