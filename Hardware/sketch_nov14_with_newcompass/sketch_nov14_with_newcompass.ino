...
//for ultrasound
elapsedMillis timeElapsed_ultrasound;
...
//HMC5883L compass
MPU6050 accelgyro_compass;
HMC5883L compass2; 
 
int16_t ax_compass, ay_compass, az_compass;
int16_t gx_compass, gy_compass, gz_compass;
int16_t mx_compass, my_compass, mz_compass;


int16_t current_ax_compass, current_ay_compass, current_az_compass;
int16_t prev_ax_compass, prev_ay_compass, prev_az_compass;

int16_t diff_ax_compass;
int16_t diff_ay_compass;
 
// accelerometer values
int accel_reading_compass;
int accel_corrected_compass;
int accel_offset_compass = 200;
float accel_angle_compass;
float accel_scale_compass = 1; // set to 0.01
 
// gyro values
int gyro_offset_compass = 151; // 151
int gyro_corrected_compass;
int gyro_reading_compass;
float gyro_rate_compass;
float gyro_scale_compass = 0.02; // 0.02 by default - tweak as required
float gyro_angle_compass;
float gyro_angle2degree_compass;
float holding_gyro_angle2degree_compass;
float gyro_angledifference_compass;
float gyro_mapped_onto_compass;
float loop_time_compass = 0.05; // 50ms loop
float angle_compass = 0.00; // value to hold final calculated gyro angle
 
// time stamp variables
int last_update_compass;
int cycle_time_compass;
long last_cycle_compass = 0;

int compassCalibrated = 0;

elapsedMillis timeElapsed_compass;
elapsedMillis timeOut_compass;
// delay in milliseconds
unsigned int interval_compass = 2000;
int bearing_int_compass ;



unsigned long prevtime;
int16_t precisionx = 250; 
int16_t precisiony = 250; 
int16_t precisionz = 200; 
...
void ultrasoundSensorPoll() //Note that ultrasound sensors read after each other
{
  ...
  timeElapsed_ultrasound = 0;
  while (timeElapsed_ultrasound < interval_ultrasound );
  timeElapsed_ultrasound = 0;
            ...
            switch(dataByte){
                ...
                case ACK_NUM:{
                  ...
                  for(index = 0; index < key_size; index++){
                      ...
                      Serial.println(parser_buffer_keys_copy[index]);
...
//for compass
void time_stamp_compass(){
  while ((millis() - last_cycle_compass) < 50){
  //delay(1);
  }
  // once loop cycle reaches 50ms, reset timer value and continue
  cycle_time_compass = millis() - last_cycle_compass;
  last_cycle_compass = millis();
}
...
//compass
  ...
  MagnetometerScaled scaled_compass = compass2.ReadScaledAxis();
  mx_compass=scaled_compass.XAxis;
  my_compass=scaled_compass.YAxis;
  mz_compass=scaled_compass.ZAxis;
...
//2) Time out calibration
 ...
 accelgyro_compass.getMotion6(&ax_compass, &ay_compass, &az_compass, &gx_compass, &gy_compass, &gz_compass);
 current_ax_compass = ax_compass;
 current_ay_compass = ay_compass;
 current_az_compass = az_compass;

 timeElapsed_compass = 0;
 while (timeElapsed_compass < interval_compass ){
   prev_ax_compass = current_ax_compass;
   prev_ay_compass = current_ay_compass;
   prev_az_compass = current_az_compass;
   accelgyro_compass.getMotion6(&ax_compass, &ay_compass, &az_compass, &gx_compass, &gy_compass, &gz_compass);
   current_ax_compass = ax_compass;
   current_ay_compass = ay_compass;
   current_az_compass = az_compass;
 ...
 timeElapsed_compass = 0;
 diff_ax_compass = abs (current_ax_compass - prev_ax_compass);
 diff_ay_compass = abs (current_ay_compass - prev_ay_compass);
 ...
 //else take readings from gyro after mapping gyroangle to degrees
  if (az_compass < 16000 && az_compass >= 15000 && diff_ax_compass <= 500 && diff_ay_compass <= 500){
    compass_scalled_reading(); 
    compass_heading();
    holding_gyro_angle2degree_compass = gyro_angle2degree_compass;
    //gyro_angle2degree=bearing; //re calibrate gyro when user is not moving
    compassCalibrated = 1;
    bearing_int_compass = int(bearing);
    timeOut_compass=0;
    
    Serial.print ("Heading angle = ");
    Serial.print (bearing_int_compass);
    Serial.println(" Degree");
    Serial.println();
  }
  else{
  // gyro_z_Axis angle calc  
    if(timeOut_compass >= 5000){  //re calibrate gyro readings under timed conditions
      compass_heading();
      holding_gyro_angle2degree_compass = gyro_angle2degree_compass;
    ...
    gyro_reading_compass = gz_compass;
    gyro_corrected_compass = (float)((gyro_reading_compass/131) - gyro_offset_compass);  // 131 is sensivity of gyro from data sheet
    gyro_rate_compass = (gyro_reading_compass * gyro_scale_compass) * -loop_time_compass;      // loop_time = 0.05 ie 50ms     
    gyro_angle_compass = gyro_angle_compass + gyro_rate_compass;
        
   gyro_angle2degree_compass = map(gyro_angle_compass, -830, 830, 0, 360);
   gyro_angle2degree_compass = constrain(gyro_angle2degree_compass, 0, 360);
   
   gyro_angle2degree_compass = gyro_angle2degree_compass*2; //map to 0 to 360 degrees
   
   if(gyro_angle2degree_compass > 359){
     gyro_angle2degree_compass = float(int(gyro_angle2degree_compass) % 360) ; 
    }
    
   gyro_angledifference_compass=(holding_gyro_angle2degree_compass - gyro_angle2degree_compass);
   holding_gyro_angle2degree_compass = gyro_angle2degree_compass;
   
   if(compassCalibrated == 1){
   gyro_mapped_onto_compass = bearing + gyro_angledifference_compass;
   }
  ...
  bearing_int_compass = int(gyro_mapped_onto_compass);
  Serial.print ("Heading angle = ");
  Serial.print (bearing_int_compass);
  Serial.println(" Degree");
  Serial.println(); 
...
//   compass_scalled_reading(); 
   compass_heading();
   gyro_angle2degree_compass = bearing; 
   bearing_int_compass = int(bearing);
   timeOut_compass = 0;
 }
